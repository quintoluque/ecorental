        ...
        {sucursales.map((sucursal) => (
          <article
            ...
            className={`tarjeta flex flex-col p-6 ${
              sucursal.casaCentral ? 'ring-1 ring-marca-300' : ''
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              ...
              {sucursal.casaCentral && (
                <span className="shrink-0 rounded-full bg-marca-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-marca-700">
                  Casa central
                </span>
              )}
            </div>
            ...
            <dl className="mt-5 space-y-3 text-sm">
              <div className="flex gap-2.5">
                <dt className="mt-0.5 shrink-0 text-marca-600">
                  <IconoUbicacion className="h-4 w-4" />
                  <span className="sr-only">Dirección</span>
                </dt>
                <dd className="text-carbon-700">
                  {sucursal.direccion ? (
                    <>
                      {sucursal.direccion}
                      {sucursal.codigoPostal && (
                        <span className="block text-carbon-500">
                          ({sucursal.codigoPostal}) {sucursal.ciudad}
                        </span>
                      )}
                    </>
                  ) : (
                    <span className="text-carbon-500">Consultá la dirección por WhatsApp</span>
                  )}
                </dd>
              </div>
            ...
            <div className="mt-5 flex flex-wrap gap-1.5">
              {sucursal.servicios.map((servicio) => (
                <span
                  key={servicio}
                  className="rounded-full bg-nieve-100 px-2.5 py-1 text-[11px] font-medium uppercase tracking-wide text-carbon-700"
                >
                  {ETIQUETAS[servicio] ?? servicio}
                </span>
              ))}
              {sucursal.centroDeSki && (
                <span className="rounded-full bg-hielo-100 px-2.5 py-1 text-[11px] font-medium uppercase tracking-wide text-hielo-700">
                  {sucursal.centroDeSki}
                </span>
              )}
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-6 inline-flex items-center justify-center gap-2 rounded-full border border-nieve-200 px-4 py-2.5 text-sm font-medium text-carbon-700 transition-colors hover:border-marca-300 hover:text-marca-700"
            >
