          ...
          {enviado ? (
            <div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-6 inline-flex items-center gap-2 rounded-full bg-carbon-900 px-6 py-3 text-sm font-semibold text-white hover:bg-carbon-700"
              >
            ...
            <form onSubmit={enviar} className="grid gap-5 sm:grid-cols-2">
              ...
              <button
                type="submit"
                disabled={enviando}
                className="rounded-full bg-carbon-900 px-7 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-carbon-700 disabled:opacity-50 sm:col-span-2 sm:justify-self-start"
              >
          ...
          <div className="tarjeta p-6">
            ...
            <ul className="mt-4 space-y-3.5 text-sm">
              <li className="flex gap-2.5">
                <IconoTelefono className="mt-0.5 h-4 w-4 shrink-0 text-marca-600" />
                <a
                  href={`tel:${empresa.telefono.replace(/[^+\d]/g, '')}`}
                  className="text-carbon-700 enlace-suave"
                >
                  {empresa.telefono}
                </a>
              </li>
              <li className="flex gap-2.5">
                <IconoWhatsapp className="mt-0.5 h-4 w-4 shrink-0 text-marca-600" />
                <a
                  href={empresa.whatsappLink}
                  target="_blank"
                  rel="noreferrer"
              ...
              <li className="flex gap-2.5">
                <span className="mt-0.5 h-4 w-4 shrink-0 text-center text-marca-600" aria-hidden="true">
                  @
                </span>
                <a href={`mailto:${empresa.email}`} className="break-all text-carbon-700 enlace-suave">
                  {empresa.email}
                </a>
              </li>
              ...
              <ul className="mt-4 space-y-2 text-sm text-carbon-700">
                {casaCentral.horarios.map((horario) => (
                  <li key={horario.dias} className="flex justify-between gap-4">
                    <span className="text-carbon-500">{horario.dias}</span>
