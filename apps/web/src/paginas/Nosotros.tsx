      ...
      <div className="contenedor py-14 lg:py-16">
        <dl className="grid gap-6 border-b border-nieve-200 pb-12 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={dato.etiqueta}>
              ...
              <dd>
                <span className="block text-4xl font-bold text-pino-700">{dato.valor}</span>
                <span className="mt-1 block text-sm text-tinta-500">{dato.etiqueta}</span>
              </dd>
            </div>
          ))}
        </dl>
        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <p className="mt-3 leading-relaxed text-tinta-700">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {actividades.map((actividad) => (
                <span
                  key={actividad}
                  className="rounded-full border border-nieve-200 bg-white px-4 py-2 text-sm font-medium text-tinta-700"
                >
                  {actividad}
                </span>
              ))}
            ...
            <div className="mt-6 space-y-4">
              {servicios.map((servicio) => (
                <div key={servicio.slug} className="tarjeta p-6">
                  ...
                  <p className="mt-2 leading-relaxed text-tinta-700">{servicio.detalle}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <ul className="mt-4 space-y-2 text-sm text-tinta-700">
                {sucursales.map((sucursal) => (
                  <li key={sucursal.slug}>
                    {sucursal.ciudad}
                    <span className="text-tinta-500"> · {sucursal.provincia}</span>
                  </li>
                ))}
              </ul>
              <Link
            ...
            <div className="tarjeta bg-pino-50 p-6">
              ...
              <div className="mt-4 space-y-2 text-sm">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block font-medium text-pino-700 hover:text-pino-600"
                >
